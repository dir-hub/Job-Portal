...
const ManageJobs = () => {
  ...
  // Function to change Joh Visibility
  ...
  return jobs ? jobs.length === 0 ? (<div className='flex items-center justify-center h-[70vh]'><p className='text-xl sm:text-2xl'>No Jobs Available or posted</p></div>) : (
    <div className='container p-4 max-w-5x1'>
      ...
      <div className='mt-4 flex justify-end'>
        <button onClick={()=>navigate('/dashboard/add-job')} className='bg-black hover:bg-gray-900
text-white py-2 px-4 rounded-md cursor-pointer'>Add new job</button>
      </div>
    </div>
  ) : <Loading />
}
