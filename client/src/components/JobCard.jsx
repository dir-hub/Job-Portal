  ...
  return (
    <div className='border border-gray-200 p-6 shadow rounded'>
        ...
        <div className='mt-4 flex gap-4 text-sm'>
            <button onClick={() => {navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className='bg-blue-600 text-white px-4 py-2 max-sm:px-2 rounded cursor-pointer'>Apply now</button>
            <button onClick={() => {navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className='text-gray-500 border border-gray-500 rounded px-4 py-2 max-sm:px-2 cursor-pointer'>Learn more</button>
        </div>
    </div>
  )
}
